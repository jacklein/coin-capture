...
// DamagePirate takes 2 damage from mainShip if hit

public class DamagePirate extends PirateShip{
	
	public DamagePirate(int scale) {
		super(scale);
		
		// specific image for SmartPirate
		pirateShipImage = new Image("file:images/smartPirate.png",scale,scale,true,true); 
	...
	public void move(MainShip mainShip) {
		...
		if (mainShip.getMoveCount() % moveAmount == 0){
			...
			// keeping track of where SmartPirate is on grid
			coords[this.getX()][this.getY()] = 1;
